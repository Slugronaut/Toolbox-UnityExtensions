...
namespace Toolbox.ToolboxEditor
{
    ...
    {
        /// <summary>
        /// Takes a full path native to the OS and coverts it to a path that is relative to this project.
        /// Used when convert full file paths to something that the Unity Editor's AssetDatabase can use.
        /// </summary>
        /// <param name="nativePath"></param>
        /// <returns></returns>
        public static string NativePathToAssetPath(string nativePath)
        {
            return nativePath.Remove(0, Application.dataPath.Length);
        }
    ...
    public static class UI
    {
        ...
        /// </summary>
        /// <param name="displayPath">The real path with the working unity project directory stripped from it. Used for nicely displaying a string that shows the folder being accessed relative to the project root.</param>
        /// <param name="realPath">The real physical path that will be used for opening directories. This can be used with IO file operations.</param>
        /// <param name="assetPath">The display path with the 'Assets' directory tacked on front. Used for performing further assetdatabase operations in the unity editor.</param>
        /// <param name="directory">The final folder name in the path that was slected.</param>
        ...
        public static bool DisplayOpenFolder(EditorWindow user, ref string displayPath, ref string realPath, ref string assetPath, ref string directory)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Select Dir", GUILayout.Width(100)))
                {
                    ...
                    LastPath = realPath;
                    string temp = realPath.Remove(0, Application.dataPath.Length).TrimStart('/');
        ...
        /// <returns></returns>
        public static bool OpenFolderButton(EditorWindow user, string title, ref string displayPath, ref string realPath, ref string assetPath, ref string directory)
        {
            bool changed = false;
            EditorGUILayout.BeginHorizontal();
            {
                if (GUILayout.Button(title, GUILayout.Width(175)))
                {
                    if (LastPath != null) realPath = LastPath;
                    realPath = EditorUtility.OpenFolderPanel("Which folder to open?", realPath, "");
                    if (string.IsNullOrEmpty(realPath)) return false;
                    LastPath = realPath;
                    string temp = realPath.Remove(0, Application.dataPath.Length).TrimStart('/');
                    if (string.IsNullOrEmpty(temp))
        ...
        /// <param name="directory">The final folder name in the path that was slected.</param>
        /// <returns><c>true</c> if the directory selected has changed, <c>false</c> otherwise."/></returns>
        public static bool SaveFileButton(EditorWindow user, string title, ref string fileName, ref string displayPath, ref string realPath, ref string assetPath, ref string directory)
        {
            bool changed = false;
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (LastPath != null) realPath = LastPath;
                    string tempFile = EditorUtility.SaveFilePanel(title, realPath, fileName, "xml");
                    if (string.IsNullOrEmpty(tempFile)) return false;

                    //get just the ending filename
                    var s = tempFile.Split('/');
                    if (s != null && s.Length > 1) fileName = s[s.Length - 1];
                    else fileName = tempFile;
                    realPath = tempFile.Replace("/" + fileName, "");

                    if (string.IsNullOrEmpty(realPath) ||  string.IsNullOrEmpty(fileName)) return false;

                    LastPath = realPath;
                    if(tempFile.Length <= Application.dataPath.Length)
                    {
                        user.ShowNotification(new GUIContent("The selected directory was invalid."));
                        //changed = true;
                        //displayPath = null;
        ...
        /// <param name="directory">The final folder name in the path that was slected.</param>
        /// <returns><c>true</c> if the directory selected has changed, <c>false</c> otherwise."/></returns>
        public static bool LoadFileButton(EditorWindow user, string title, ref string fileName, ref string displayPath, ref string realPath, ref string assetPath, ref string directory)
        {
            bool changed = false;
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (LastPath != null) realPath = LastPath;
                    var tempFile = EditorUtility.OpenFilePanel("What file to load?", fileName, "xml");
                    if (string.IsNullOrEmpty(tempFile)) return false;
